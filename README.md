...
3. מנהל מערכת מייצא בקשות ממתינות ל-USB

### שלב 4: חתימה חיצונית (רשת מחוברת לאינטרנט)

1. כונן USB מחובר למערכת חיצונית
...
### שלב 5: ייבוא כרטיסים (ברשת המבודדת)

1. כונן USB מוחזר לרשת המבודדת
...
**העברת USB חזרה לרשת המבודדת**

**אחר הצהריים** (ברשת המבודדת):
```bash
# ייבוא כרטיסים
./bin/proxy-server import --input /mnt/usb/tickets
```

**הודעה למשתמשים**: עדכונים זמינים כעת לניסיון חוזר

### תחזוקה שבועית

- הרצת ניקוי מטמון להסרת כרטיסים שפג תוקפם
...
1. **לוח בקרה Web**: ניטור בזמן אמת של תור ומטמון
2. **חתימה אוטומטית**: מחזורי ייצוא/חתימה/ייבוא מתוזמנים
3. **זמינות גבוהה**: מספר מופעי Proxy Server עם מטמון משותף
4. **אבטחה משופרת**: אימות TLS הדדי, חתימת בקשות
5. **אנליטיקה**: מעקב אחר שיעור הצלחת עדכונים, מלאי Client Macs
...
**אתגרים**:
- ⚠️ דורש תיאום ידני יומיומי (העברת USB)
...
מסמך זה מספק סקירה מלאה של הארכיטקטורה, תהליכי העבודה, דרישות המערכת, ושיקולי אבטחה. ניתן להציג אותו למנהלים לאישור לפני תחילת הפיתוח.
